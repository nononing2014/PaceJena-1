...
public class PaceJena extends DefaultHandler implements LexicalHandler {
	...
	List<Ontology> importList = new ArrayList<Ontology>();
	...
	public PaceJena(String ontologyFileNames[]) {
	  //PaceJena o = new PaceJena();
	  readOntology(ontologyFileNames);
	...
	public void readOntology(String ontologyFileNames[]) {
	...
	try {
		...
		for(String ontologyFileName: ontologyFileNames){
			...
			importList.add(currentOntology);
		...
		pass = 2;
		for(Ontology o: importList){
			currentOntology = o;
			textBuffer = null;
			saxParser.parse(new File(o.filePath), this); // process the rest
			//stream.close();
			//stream = getClass().getResourceAsStream(ontologyFileName);
  ...
  public Vector<OwlClass> getLearningOrderClasses(String name) {
	for(Ontology o: importList)
		if(o.learningOrderHash.get(name) != null)
			return o.learningOrderHash.get(name);
	return null;
  }
  ...
  public String[] getLearningOrderIDs(Ontology ont) {
    Enumeration<String> e = ont.learningOrderHash.keys();
  ...
  public String[] getLearningOrderNames(String id) {
	...
	void printLearningOrders() {
	  System.out.println("\nLearning Orders:");
	  String[] ids = getLearningOrderIDs(currentOntology);
	...
	public OwlClass findClass(String name) {
	  ...
	  for(Ontology o: importList)
		if(o.owlClassHash.get(name) != null)
			return o.owlClassHash.get(name);
	...
	public PropertyClass findProperty(String name) {
	  ...
	  for(Ontology o: importList)
		if(o.propertyHash.get(name) !=null)
			return o.propertyHash.get(name);
	...
	public DataType findDatatype(String name) {
	  if (name.startsWith("#")) name = name.substring(1); 
	  for(Ontology o: importList)
	    if(o.dataTypeHash.get(name)!=null)
		  return o.dataTypeHash.get(name);
	  error("DataType " + name + " missing");
	  return null;   
	}
	...
	public Relation findRelation(String name) {
		  ...
		  if(iri.split("#").length >= 2)
			iri = iri.split("#")[1];
		  for(Ontology o:importList)
			if(o.newRelationHash.get(iri)!=null)
				return o.newRelationHash.get(iri);
		  error("Relation " + name + " missing");
		  return null;   
	}
